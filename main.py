...
async def reset_bot_state(key, iv, region):
    ...
    try:
        ...
        print("✅ Bot state reset - left squad..! [b][i][FFFF00 YT @ehmunna-1")
        return True
        ...
        for account_uid, password in accounts_data.items():
            try:
                ...
                if join_packet:
                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
                    success_count += 1
                    print(f"✅ Join sent from authenticated account: [b][i][FFFF00 YT @ehmunna-1{account_uid}")
...
async def handle_badge_command(cmd, inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        ...
        for i in range(spam_count):
            await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)
            print(f"✅ Sent /{cmd} request #{i+1} with badge[b][i][FFFF00 YT @ehmunna-1 {badge_value}")
            await asyncio.sleep(0.1)
        
        success_msg = f"[b][b][b][FFFFFF]✅ Successfully Sent [b][i][FFFF00 YT @ehmunna-1 {spam_count} Join Requests!\n🎯 Target: {target_uid}\n🏷️ Badge: {badge_value}\n"
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if spam_count >= 150:
            completion_msg = f"[b][b][b][FFFFFF]✅ Reject Spam Completed Successfully for ID [b][i][FFFF00 YT @ehmunna-1{target_uid}\n✅ Total packets sent: {spam_count * 2}\n"
        else:
...
[b][b][FFC0CB]youtube: @ehmunna-1[/b]

[b][FFC0CB]subcribe: my_channel[00FFFF]
 
[b][b][FFA500]telegram: @ehmunna9[/b]

[b][FFA500]telegram contact: @ehmunna9[A52A2A]
 
Enjoy the bot my friend.......

[C][b][00FFFF] Created by MUNNA 
Modified by - PROGRAMAR X
"""
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                evo_custom_spam_running = False
                                evo_custom_spam_task.cancel()
                                success_msg = f"[b][FFFFFF]✅ SUCCESS! Evolution custom spam stopped successfully! YT @ehmunna-1\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = """[b][00FFFF]✨ EH MUNNA GOST BOT ✨
                        [00FFFF]WELCOME!           গরীবের এলাকায়  """
...
-> [00FFFF]Join Req Pro Badge"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # Footer
                            footer = """[C][b][1E90FF]╭━━━━━━━━━╮
[1E90FF]┃ [00FFFF]CREDITS: EH MUNNA [1E90FF]
[1E90FF]┃ [00FFFF]YOUTUBE @ehmunna-1[1E90FF]
[1E90FF]┃ [00FFFF]DEVELOPER: PROGRAMAR X [1E90FF]
[1E90FF]┃ [00FFFF]BOT STATUS: ONLINE[1E90FF]
[1E90FF]┃ [00FFFF]EH MUNNA [1E90FF]
[1E90FF]╰━━━━━━━━━╯"""
...
async def MaiiiinE():
    Uid , Pw = '4403099560','12E36803576A44EE6FEE000328793146AB9288E4153D1CF01B866B70316A5548'
    ...
    os.system('clear')
    print("Initializing EH MUNNA Bot...")
    print("┌────────────────────────────────────┐")
    print("│ EH │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
    ...
    print("Connecting to Free Fire servers...")
    print("┌────────────────────────────────────┐")
    print("│ MUNNA PROGRAMAR X │")
    print("└────────────────────────────────────┘")
    time.sleep(0.5)
    os.system('clear')

    print("☠️ EH MUNNA BOT - ONLINE")
    print("┌────────────────────────────────────┐")
    print("│ EH MUNNA │")
    print("└────────────────────────────────────┘")
    print(f"🔹 UID: {TarGeT}")
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using EH MUNNA Bot!")
    sys.exit(0)
